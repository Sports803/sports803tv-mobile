...
export default function HomeScreen() {
  ...
  return <ScreenContainer containerClassName="bg-background" className="px-4"><FlatList key={`events-${columns}`} data={mainEvents} numColumns={columns} columnWrapperStyle={mainEvents.length ? { gap: gridGap } : undefined} keyExtractor={(item) => item.id} renderItem={({ item }) => <EventCard event={item} width={cardWidth} favorite={favorites.includes(item.id)} onFavorite={() => void setFav(item.id)} onOpen={() => openEvent(item)} />} ListHeaderComponent={header} ListFooterComponent={footer} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void load(true); }} tintColor={palette.red} />} ListEmptyComponent={loading ? <Skeletons /> : homeLayout.showFixtures ? <View style={{ paddingVertical: 30, alignItems: "center" }}><Text style={{ color: palette.text, fontSize: 16, fontWeight: "800" }}>No upcoming fixtures found</Text><Text style={{ color: palette.muted, marginTop: 6, textAlign: "center" }}>{query.trim() ? "Try another team, league, or sport." : "Pull down to refresh the live fixture feed."}</Text></View> : null} contentContainerStyle={{ paddingBottom: 28 }} /></ScreenContainer>;
}
