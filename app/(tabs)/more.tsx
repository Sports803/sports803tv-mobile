...
export default function MoreScreen() {
  ...
  useEffect(() => { void load(); void getDataSaver().then(setDataSaverState); void getAnalyticsConsent().then(setAnalyticsConsentState); }, [load]);
    ...
    <ScreenContainer containerClassName="bg-background" className="px-4">
      <FlatList
        ...
        ListHeaderComponent={<View style={{ paddingTop: 14, paddingBottom: 28 }}>
          ...
          <View style={card}><Text style={{ color: "#F7F8FC", fontSize: 17, fontWeight: "800" }}>Notifications</Text><Text style={{ color: "#9AA6BE", marginTop: 8, lineHeight: 19 }}>Allow local reminders 15 minutes before your selected matches begin.</Text><Pressable onPress={async () => Alert.alert("Notifications", (await requestNotificationPermission()) ? "Notifications are enabled." : "Notifications were not enabled on this device.")} style={{ marginTop: 14 }}><Text style={{ color: "#36D399", fontWeight: "900" }}>Enable match alerts</Text></Pressable></View>
